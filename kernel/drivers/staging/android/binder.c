			       ...
			       struct binder_transaction_data *tr, int reply)
{
	...
	if (!reply && (binder_perf_evalue & BINDER_PERF_SEND_COUNTER))
	{
		...
		{
			if (!strcmp(proc->bc_stats[i].service, "") && (0 == proc->bc_stats[i].code[0]))
			{
				     ...
				     struct binder_transaction *t)
{
	...
	if (t->buffer == NULL) {
#ifdef BINDER_MONITOR
		seq_printf(m, " start %lu.%02lu android %d-%02d-%02d %02d:%02d:%02d.%03lu"
			" buffer free\n",
			(unsigned long)t->timestamp.tv_sec,
			(t->timestamp.tv_nsec / NSEC_PER_USEC),
			(tm.tm_year + 1900), (tm.tm_mon + 1), tm.tm_mday,
...
#ifdef BINDER_MONITOR
	seq_printf(m, " size %zd:%zd data %p auf %d start %lu.%02lu"
		   " android %d-%02d-%02d %02d:%02d:%02d.%03lu\n",
		   t->buffer->data_size, t->buffer->offsets_size,
					...
					struct binder_transaction_log_entry *e)
{
	...
	seq_printf(m,
		   ...
		   "dex %d start %lu.%02lu android %d-%02d-%02d %02d:%02d:%02d.%03lu\n",
...
static void binder_perf_stats_bct_clean(void)
{
	...
	hlist_for_each_entry(proc, pos, &binder_procs, proc_node)
	{
		...
		for (i = 0; i < BC_STATS_NR; i++)
		{
			strcpy(proc->bc_stats[i].service, "");
			memset(&proc->bc_stats[i].code[0], 0, sizeof(proc->bc_stats[i].code));
			memset(&proc->bc_stats[i].code_num[0], 0, sizeof(proc->bc_stats[i].code_num));
		}
	}
	if (do_lock)
		binder_unlock(__func__);
