...
$(MTK_ROOT_CONFIG_OUT)/ProjectConfig.mk: $(MTK_PROJECT_CONFIGS)
	...
	@if [ "$(TCL_TELEPHONY_SUPPORT)" = "true" ]; then \
	sed -i 's/MTK_TB_APP_CALL_FORCE_SPEAKER_ON =\s*.*/MTK_TB_APP_CALL_FORCE_SPEAKER_ON = yes/g' $$@; \
	sed -i 's/MTK_TTY_SUPPORT =\s*.*/MTK_TTY_SUPPORT = yes/g' $$@; \
	sed -i 's/MTK_VT3G324M_SUPPORT =\s*.*/MTK_VT3G324M_SUPPORT = yes/g' $$@; \
	sed -i 's/TCT_SETTINGS_CALL =\s*.*/TCT_SETTINGS_CALL = yes/g' $$@; \
	sed -i 's/RESOURCE_OVERLAY_SUPPORT =\s*.*/RESOURCE_OVERLAY_SUPPORT = generic navbar tablet/g' $$@; \
	else \
	sed -i 's/MTK_TB_APP_CALL_FORCE_SPEAKER_ON =\s*.*/MTK_TB_APP_CALL_FORCE_SPEAKER_ON = no/g' $$@; \
	sed -i 's/MTK_TTY_SUPPORT =\s*.*/MTK_TTY_SUPPORT = no/g' $$@; \
	sed -i 's/MTK_VT3G324M_SUPPORT =\s*.*/MTK_VT3G324M_SUPPORT = no/g' $$@; \
	sed -i 's/TCT_SETTINGS_CALL =\s*.*/TCT_SETTINGS_CALL = no/g' $$@; \
	sed -i 's/RESOURCE_OVERLAY_SUPPORT =\s*.*/RESOURCE_OVERLAY_SUPPORT = generic navbar tablet generic_3GDATASMS/g' $$@; \
	fi
) \
