	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			...
			else
			{
				...
				mutex_unlock(&kxtj2_1009_mutex);
				sscanf(buff, "%x %x %x", &gsensor_data->values[1], 
					&gsensor_data->values[0], &gsensor_data->values[2]);	
//				printk("<2>""gsensor_data->values[1]=%d,gsensor_data->values[0]=%d,gsensor_data->values[2]=%d\n",gsensor_data->values[1],gsensor_data->values[0],gsensor_data->values[2]);	
				gsensor_data->values[1] = -gsensor_data->values[1];
//				printk("<2>""gsensor_data->values[1]=%d,gsensor_data->values[0]=%d,gsensor_data->values[2]=%d\n",gsensor_data->values[1],gsensor_data->values[0],gsensor_data->values[2]);	
