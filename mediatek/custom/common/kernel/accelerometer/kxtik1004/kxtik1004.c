...
/*----------------------------------------------------------------------------*/
static int KXTIK1004_i2c_read_block(struct i2c_client *client, u8 addr, u8 *data, u8 len)
{
        u8 beg = addr;
...
static int KXTIK1004_SetDataResolution(struct kxtik1004_i2c_data *obj)
{
	...
	if(KXTIK1004_i2c_read_block(obj->client, KXTIK1004_REG_DATA_RESOLUTION, databuf, 0x01))
	{
...
static int KXTIK1004_ReadData(struct i2c_client *client, s16 data[KXTIK1004_AXES_NUM])
{
	...
	else if((err = KXTIK1004_i2c_read_block(client, addr, buf, 0x06)))
	{
		GSE_ERR("error: %d\n", err);
	}
	else
...
static int KXTIK1004_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	if(KXTIK1004_i2c_read_block(client, addr, databuf, 0x01))
	{
...
static int KXTIK1004_SetDataFormat(struct i2c_client *client, u8 dataformat)
{
	...
	if(KXTIK1004_i2c_read_block(client, KXTIK1004_REG_DATA_FORMAT, databuf, 0x01))
	{
		printk("kxtik1004 read Dataformat failt \n");
		return KXTIK1004_ERR_I2C;
	}
...
static int KXTIK1004_SetBWRate(struct i2c_client *client, u8 bwrate)
{
	...
	if(KXTIK1004_i2c_read_block(client, KXTIK1004_REG_BW_RATE, databuf, 0x01))
	{
...
static ssize_t show_register_value(struct device_driver *ddri, char *buf)
{
		...
		if(KXTIK1004_i2c_read_block(obj->client, i2c_dev_reg, databuf, 0x01))
		{
			GSE_ERR("read power ctl register err!\n");
			return KXTIK1004_ERR_I2C;
		}
