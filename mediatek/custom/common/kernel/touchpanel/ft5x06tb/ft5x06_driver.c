...
static int pop8_freq_hop_check_status=0;
static int tpd_need_do_freq_hop_check_en=-1;
extern void set_do_chrdet_in_touch_driver(void (*func)(void));
...
#define TPD_THREAD_RESTART 1
#if TPD_THREAD_RESTART
static int bad_tpd_interrupt_num=0;
static int tpd_thread_reset(void);
//static struct workqueue_struct *tpd_thread_reset_wq;
static struct work_struct *tpd_thread_reset_work;
#endif
//register define
...
//=========================add by wfy for interrupt debug
static int enable_int_debug = 0;
//----------------------------------------
...
int ft5x06_freq_hop_check_en(struct i2c_client *i2c_client,int val)
{
	int ret;

	if(i2c_client==NULL) return -1;

	ret = ft5x0x_write_reg(i2c_client, 0x8b, val);
//	printk("<2>""----set ft5x06 reg 0x8b to %d,ret =----\n",val);
	if(ret < 0){
		TPD_INFO("Suspend can not write 0xAB\n");
		return ret;
	}

	pop8_freq_hop_check_status=val;
	return 0;
}

extern kal_bool upmu_is_chr_det(void);

void do_chardet_touch_work(void)
{
	if(upmu_is_chr_det()==KAL_TRUE){

		printk("<2>""%s: charger is pug in,enable freq hop check\n",__func__ );
		if(tpd_is_suspend)
			tpd_need_do_freq_hop_check_en=1;
		else
			ft5x06_freq_hop_check_en(i2c_client,1);
	}else{
		printk("<2>""%s: charger is pug out,disable freq hop check\n",__func__ );
		if(tpd_is_suspend)
			tpd_need_do_freq_hop_check_en=0;
		else
			ft5x06_freq_hop_check_en(i2c_client,0);
	}
}
...
//=========================add by wfy for interrupt debug
static ssize_t intdbg_store(struct kobject *kobj,struct kobj_attribute *attr,const char * buf, size_t n)
{
	int symbol = -1;
	int ret;
	...
	symbol = myatoi(buf);
	if (symbol>=0) {
		enable_int_debug = symbol;
		printk("<2>""%s touch interrupt debug!!!\n",enable_int_debug?"open":"close");
	}

	return n;
}

static ssize_t intdbg_show(struct kobject *kobj,struct kobj_attribute *attr,char * buf)
{
	u8 freq_hop=0xff;
        if(ft5x0x_read_reg(i2c_client, 0x8b,&freq_hop)<0){
                printk("<2>""read reg 0x8b err\n");
                freq_hop=0xff;
        };
        return sprintf(buf, "touch interrupt debug is %s, freq hop status=%d\n", enable_int_debug?"open":"closed",freq_hop);
}

#if TPD_THREAD_RESTART
static ssize_t tpd_thread_store(struct kobject *kobj,struct kobj_attribute *attr,const char * buf, size_t n)
{
	int ret;

	if(strncmp(buf,"start",5)==0){
		ret=tpd_thread_reset();
		printk("<2>""start tpd thread,ret=%d\n",ret);
	}else if(strncmp(buf,"stop",4)==0){
		if(thread==NULL){
			printk("<2>""tpd thread is not exist\n");
			return n;
		}
	...
	//	if(ret==0)
			thread=NULL;
		printk("<2>""stop tpd thread,ret=%d\n",ret);
	}else if(strncmp(buf,"wake",4)==0){
		if(!IS_ERR(thread))
			wake_up_process(thread);
		printk("<2>""tpd thread wake up now\n");
	}else
		printk("<2>""cmd must \"stop\" or \"start\"!!\n");
	return n;
}

static ssize_t tpd_thread_show(struct kobject *kobj,struct kobj_attribute *attr,char * buf)
{
	if(thread==NULL)
		return sprintf(buf,"tpd thread not exist\n");

	if(IS_ERR(thread))
		return sprintf(buf,"tpd thread exist but error\n");

	return sprintf(buf,"tpd thread exist, state:%x,pid:%d,tgid:%d,flag:%d,ptrace:%d,on_cpu:%d,on_rq:%d,thread:%d\n",thread->state,thread->pid,thread->tgid,thread->flags,thread->ptrace,thread->on_cpu,thread->on_rq,tsk_fork_get_node(thread));
}
#endif
//-------------------------------------------------------
...
//=========================add by wfy for interrupt debug
static struct kobj_attribute intdbg_attr = { \
	.attr = { \
	.name = __stringify(intdbg), \
	.mode = 0644, \
	}, \
	.show = intdbg_show, \
	.store = intdbg_store, \
...
#if TPD_THREAD_RESTART
static struct kobj_attribute tpd_thread_attr = { \
	.attr = { \
	.name = __stringify(tpd_thread), \
	.mode = 0644, \
	}, \
	.show = tpd_thread_show, \
	.store = tpd_thread_store, \
};
#endif
//----------------------------------------------------
...
static int touch_event_handler(void *unused)
{
	...
	while(tpd_register_flag==0){
		printk("<2>""%s: tpd driver not register finish,must wait!!!\n",__func__);
		tpd_flag = 0;
		mdelay(10);
	}
	 ...
	 do
	 {
		...
		if (tpd_touchinfo(&cinfo, &pinfo))
		{
			TPD_DEBUG("point_num = %d\n",point_num);
			if(enable_int_debug)
				printk("<2>""point_num = %d ",point_num);
			...
			if(point_num >0) 
			{
				...
				while(i<point_num)
				{
					...
					input_x = cinfo.x[i];
					input_y = cinfo.y[i];
//					printk("[dexiao before cali]x:%d, y:%d.\n", input_x, input_y);
//					input_x = input_x * 1023 / 599;
//					input_y = input_y * 599 / 1023;
					//input_y = 599 - input_y;
					tpd_calibrate_driver(&input_x, &input_y);
//					printk("[dexiao after cali]x:%d, y:%d.\n", input_x, input_y);
...
//	mdelay(100);
	...
	{ 
		ret = PTR_ERR(thread);
		printk("<2>"" failed to create kernel thread: %d\n", ret);
		return ret;
	}
	printk("<2>""%s: restart touch event handler  ok !!!\n",__func__);
	return 0;
}


static void tpd_thread_reset_func(struct work_struct *work)
{
	tpd_thread_reset();
}
#endif
...
static void tpd_eint_interrupt_handler(void)
{
	...
	if(enable_int_debug)
		printk("<2>""%s: TPD interrupt has been triggered, tpd_flag=%d\n",__func__,tpd_flag);
	
#if TPD_THREAD_RESTART
	if(tpd_flag && tpd_register_flag){
		if(++bad_tpd_interrupt_num>100){
			bad_tpd_interrupt_num=0;
			printk("<2>""%s: tpd thread is ill,reset now!!!\n",__func__);
			schedule_work(tpd_thread_reset_work);
		}
		if(enable_int_debug)
			printk("<2>""%s: bad_tpd_interrupt_num=%d\n",__func__,bad_tpd_interrupt_num);
...
i2c_transfer_sucess:
	...
	tpd_thread_reset_work = kzalloc(sizeof(typeof(*tpd_thread_reset_work)), GFP_KERNEL);
	if (!tpd_thread_reset_work) {
		TPD_INFO("create work queue error, line: %d\n", __LINE__);
		return -1;
	}
	INIT_WORK(tpd_thread_reset_work, tpd_thread_reset_func);
	...
	set_do_chrdet_in_touch_driver(do_chardet_touch_work);
	do_chardet_touch_work();
...
static int tpd_resume(struct i2c_client *client)
{
	...
	tpd_is_suspend=0;
	if(tpd_need_do_freq_hop_check_en!=-1){
		printk("<2>""%s: charger is pug %s in touch suspend,%s freq hop check now\n",__func__,(tpd_need_do_freq_hop_check_en==1)?"in":"out",(tpd_need_do_freq_hop_check_en==1)?"enable":"disable" );
		ft5x06_freq_hop_check_en(i2c_client, tpd_need_do_freq_hop_check_en);
		tpd_need_do_freq_hop_check_en=-1;
	}else{
		ft5x06_freq_hop_check_en(i2c_client, pop8_freq_hop_check_status);
	}
	return retval;
}
