...
static int fw_len;//= sizeof(CTPM_FW);
...
static unsigned char CTPM_FW_JD[] = {
	#include "G07_5436I_ID85_V21_20140122_app.i"
};

static unsigned char CTPM_FW_YJ[] = {
	#include "G07_5436I_ID80_V61_20131225_app.i"
};
...
u8 fts_ctpm_update_project_setting(struct i2c_client *client)
{
	...
	u32 temp;
	u32 lenght;
	u8 packet_buf[4];	//FTS_PACKET_LENGTH + 6
	...
	printk("is_5336_new_bootloader = %d\n",is_5336_new_bootloader);
	...
	printk("-----------------------pProjectCode[4] = %d\n",pProjectCode[4]);
	return pProjectCode[4];
}
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	else if(0x80 == uc_tp_id)
	{
		printk("0x80 == uc_tp_id\n");
		fw_len = sizeof(CTPM_FW_YJ);
		CTPM_FW= CTPM_FW_YJ;
	}
	else
		printk("  uc_tp_id = %d\n",uc_tp_id);
			  ...
			  u32 dw_lenth)
{
	...
	if(is_5336_new_bootloader == BL_VERSION_LZ4 || is_5336_new_bootloader == BL_VERSION_Z7 )
	{
		for (i = 0; i<6; i++)
		{
			if (is_5336_new_bootloader  == BL_VERSION_Z7 && DEVICE_IC_TYPE==IC_FT5606) 
			{
				temp = 0x7bfa + i;
			}
	...
	else if(is_5336_new_bootloader == BL_VERSION_GZF)
	{
		for (i = 0; i<12; i++)
		{
			if (is_5336_fwsize_30 && DEVICE_IC_TYPE==IC_FT5606) 
			{
				temp = 0x7ff4 + i;
			}
			else if (DEVICE_IC_TYPE==IC_FT5606) 
			{
				temp = 0x7bf4 + i;
			}
			packet_buf[2] = (u8)(temp>>8);
					...
					const char *buf, size_t count)
{
	...
	if (1 == val)
	{
		printk("JD Firmware upgrade\n");
		fw_len = sizeof(CTPM_FW_JD);
		CTPM_FW= CTPM_FW_JD;
	}
	else if(2 == val)
	{
		printk("YJ Firmware upgrade\n");
		fw_len = sizeof(CTPM_FW_YJ);
		CTPM_FW= CTPM_FW_YJ;
	}
#endif	
