...
static void lcm_init(void)
{
    ...
    printf("TDT+CPT %s, LK \n", __func__);
#else
    printk("TDT+CPT %s, kernel", __func__);
#endif
	//tps65640_en();
...
static unsigned int lcm_compare_id(void)
{
	...
	id = (id1<<1)|(id2);
#ifdef BUILD_LK
    printf("Evan:TDT+CPT id1=%d,id2=%d,id=0x%x\n",id1,id2,id);
#else
    printk("Evan:TDT+CPT id1=%d,id2=%d,id=0x%x\n",id1,id2,id);
#endif
	lcd_reset(0);
    lcd_power_en(0);
    MDELAY(10);
...
LCM_DRIVER a080ean01_TDT_CPT_dsi_vdo_lcm_drv = 
{
    .name           = "a080ean01_TDT_CPT_dsi_vdo",
