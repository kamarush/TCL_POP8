...
#endif
#if (TCL_PROTO_PCB)
	mt_set_gpio_out(GPIO_LCM_PWR, GPIO_OUT_ONE); //for proto only
#endif
	upmu_set_rg_vcam_af_vosel(0x7); 
...
#endif
#if (TCL_PROTO_PCB)
    mt_set_gpio_out(GPIO_LCM_PWR, GPIO_OUT_ZERO);//for proto only
#endif
	upmu_set_rg_vcam_af_vosel(0); 
	upmu_set_rg_vcam_af_en(0);

	//upmu_set_rg_vibr_vosel(0x0); //modify VIBR TO 3.3V
...
#ifdef BUILD_LK
    printf("Trust+AUO %s, LK \n", __func__);
#else
    printk("Trust+AUO %s, kernel", __func__);
#endif
	//tps65640_en();
...
static unsigned int lcm_compare_id(void)
{
	...
	mt_set_gpio_pull_select(GPIO_HALL_2_PIN, GPIO_PULL_DOWN);
	...
	id = (id1<<1)|(id2);
#ifdef BUILD_LK
    printf("TRUST+AUO id1=%d,id2=%d,id=0x%x\n",id1,id2,id);
#else
    printk("TRUST+AUO id1=%d,id2=%d,id=0x%x\n",id1,id2,id);
#endif
	lcd_reset(0);
    lcd_power_en(0);
    MDELAY(10);
