...
static unsigned char CTPM_FW_OF[] = {
	#include "POP8_5506_ID51_V65_20140327_app.i"	/*[BUGFIX] Mod by TCTSZ.WH,2014-4-10,Upgrade Ofilm to V65*/
};

static unsigned char CTPM_FW_JD[] = {
//	#include "POP8_5506_ID85_V1B_20140122_app.i"
	#include "POP8_5506_ID85_V16_20131231_app.i"
};
...
//	int fw_len = sizeof(CTPM_FW);
	...
	*/
	...
	if ((CTPM_FW[fw_len - 8] ^ CTPM_FW[fw_len - 6]) == 0xFF
		...
		i_ret = fts_ctpm_fw_upgrade(client, pbt_buf, fw_len);//sizeof(CTPM_FW)
		if (i_ret != 0)
...
static u16 fts_ctpm_get_vid_fwid_from_Boot(struct i2c_client *client)
{
    u8 reg_val[2] = {0};
    u32 i = 0, j_do;
    u32 packet_number;
    ...
    u16 usVidFwID = 0;
    int i_ret;
    //u8 is_new_bootloader = 0;
    int fw_filenth = fw_len;//sizeof(CTPM_FW);
    struct Upgrade_Info upgradeinfo;

    mtk_wdt_enable(WK_WDT_DIS); //added by zhengdao
   ...
   //else
       temp = 0x0754;
   packet_buf[2] = 0x07;
   packet_buf[3] = 0x54;//(u8)temp;

   ft5x0x_i2c_Read(client, packet_buf, 4,reg_val, 2);
   printk("Vendor ID = %x %x \n", reg_val[0], reg_val[1]);
#if 0    
   ...
   //else
       temp = 0x0770;
   packet_buf[2] = 0x07;//(u8)(temp>>8);
   packet_buf[3] = 0x70;//(u8)temp;

   ft5x0x_i2c_Read(client, packet_buf, 4,reg_val, 1);
   ...
   usVidFwID |= (u8)reg_val[0];
    ...
    mtk_wdt_enable(WK_WDT_EN); //added by zhengdao  
    return usVidFwID;
}
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	ret=ft5x0x_read_reg(client, FT5x0x_REG_FW_VER, &uc_tp_fm_ver);
	printk(" fw_ver_ret = %d,uc_tp_fm_ver=%d\n",ret,uc_tp_fm_ver);
	ret = ft5x0x_read_reg(client, 0xA8, &uc_tp_id);
	printk("----tp_id_ret = %d, uc_tp_id = %d\n",ret,uc_tp_id);

	if ((uc_tp_fm_ver == 0) || (uc_tp_id == 0)){
		printk("Wrong version number or factory ID read form FW, get factor ID from bootloader!\n");
		uc_tp_id = fts_ctpm_update_project_setting(client);
		//uc_tp_id = fts_ctpm_get_vid_fwid_from_Boot(client);
	}

	if(0x85 == uc_tp_id){
		printk("0x85 == uc_tp_id\n");
		fw_len = sizeof(CTPM_FW_JD);
		CTPM_FW= CTPM_FW_JD;
	}
	else if(0x51 == uc_tp_id)
	{
		printk("0x51 == uc_tp_id\n");
		fw_len = sizeof(CTPM_FW_OF);
		CTPM_FW= CTPM_FW_OF;
	}
	else
		printk("  uc_tp_id = %d\n",uc_tp_id);
