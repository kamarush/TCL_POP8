...
static s32 gtp_init_panel(struct i2c_client *client)
{
    ...
    GTP_INFO("config[3]=%x,config[4]=%x,config[5]=%x,config[6]=%x,config[8]=%x\n",config[3],config[4],config[5],config[6],config[8]);
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (!int_type)
    {
        mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, EINTF_TRIGGER_FALLING, tpd_eint_interrupt_handler, 1);	//CUST_EINT_POLARITY_HIGH
    }
    else
    {
        mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, EINTF_TRIGGER_FALLING, tpd_eint_interrupt_handler, 1);
...
static void tpd_eint_interrupt_handler(void)
{
	GTP_DEBUG("TPD interrupt has been triggered\n");
