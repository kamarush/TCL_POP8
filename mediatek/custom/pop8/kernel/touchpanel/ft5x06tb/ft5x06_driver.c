...
{
	return sprintf(buf, "touch interrupt debug is %s\n", enable_int_debug?"open":"closed");
}
...
#else
		printk("<2>""tpd go to suspend!!\n");
		mt_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
		data = 3;
		ret = ft5x0x_write_reg(i2c_client, 0xa5, data);
		if(ret < 0)
			TPD_INFO("Suspend can not write 0xAB\n");
