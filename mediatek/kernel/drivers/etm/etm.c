...
/* 
 * trace_start_dormant: Start trace by the dormant driver.
 */
void trace_start_dormant(void)
{
	...
	/* AHBAP_EN to enable master port, then ETR could write the trace to bus */
#if 0
	mt65xx_reg_sync_writel(DEM_UNLOCK_MAGIC, DEM_UNLOCK);
#endif
...
/* 
 * trace_start_dormant: Stop trace by the dormant driver.
 */
void trace_stop_dormant(void)
{
	...
	/* power down */
	for (i = 0; i < tracer.nr_etm_regs; i++) {
		cs_cpu_write(tracer.etm_regs[i], 0x0, 1);
	}
...
static int etm_probe(struct platform_device *pdev)
{
	...
	pr_info("ETM id is 0x%08x\n", config_code);
	pr_info("ETM config_code is 0x%08x\n", config_code);
	pr_info("ETM config_code_extension is 0x%08x \n", config_code_extension);
	pr_info("ETM system_config is 0x%08x \n", system_config);	  
