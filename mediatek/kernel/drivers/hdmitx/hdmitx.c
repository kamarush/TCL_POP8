...
static int hdmi_update_kthread(void *data)
{
    ...
    for( ;; ) {
        ...
        HDMI_LOG("enter hdmi_update_kthread \n");
        hdmi_update_impl();
        atomic_set(&hdmi_update_event, 0);
    ...
    for( ;; ) {
           ...
           !(needStopWdma1 && hdmi_overlay_status == HDMI_OVERLAY_STATUS_STOPPED))
        {
            ...
            if(needStopWdma1)
            {
                ...
                HDMI_LOG("needStopWdma1 = %d \n", needStopWdma1);
                // Stop WDMA1

                wdma1Config.enable = 0;
                disp_path_config_mem_out_new(&wdma1Config);
                ...
                #else
                hdmi_buffer_lcdw_id_tmp = hdmi_buffer_lcdw_id;
...
void hdmi_set_rdma_address(int bufferIndex)
{
    ...
    //HDMI_FUNC();
    //HDMI_LOG("dp_mutex_dst = %d, bufferindex = %d \n",dp_mutex_dst,bufferIndex);
    if (-1 != dp_mutex_dst)
    {
...
{
    //RET_VOID_IF(p->output_mode == HDMI_OUTPUT_MODE_DPI_BYPASS);
     dpi_setting_res((u8)hdmi_params->init_config.vformat);
#if 0
...
int hdmi_display_path_overlay_config(bool enable)
{
    ...
    else
    {
        ...
        hdmi_overlay_status = HDMI_OVERLAY_STATUS_STOPPING;
        disp_path_release_mutex();
        wait_event_interruptible_timeout(hdmi_reg_update_wq, atomic_read(&reg_update_event), HZ);
    }
    ...
    if(enable)
    {
        if(p->is_clock_on == true)
        {
            ...
            return;
				...
				hdmi_config_pll(hdmi_params->init_config.vformat);
...
/*static*/ void hdmi_suspend(void)
{
    ...
    //OUTREG32(0xF400D04C, 0x00000000);
    //OUTREG32(0xF400D04C, 0x41);
    //DISP_LCMEnable(true);
...
extern int OVLStart(void);

// stop overlay module
extern int OVLStop(void);
...
/*static*/ void hdmi_resume(void)
{
    ...
    //OVLStop();
    //disp_path_release_mutex();
    //wait_event_interruptible(reg_update_wq, 1);
    //printk("hdmi_resume wait for ovl stop \n");
    ...
    if(-1 != dp_mutex_dst)
    {
        HDMI_LOG("Init bls and color \n");
        disp_path_get_mutex_(dp_mutex_dst);
        disp_bls_init(p->hdmi_width, p->hdmi_height);
        DpEngine_COLORonInit();
        DpEngine_COLORonConfig(p->hdmi_width,p->hdmi_height);
        disp_path_release_mutex_(dp_mutex_dst);
    }
    #else
    //hdmi_dst_display_path_config(true);
    ...
    if (hdmi_mva_r != 0)
    {
        hdmiSourceAddr = hdmi_mva_r + p->hdmi_width * p->hdmi_height * hdmi_bpp * hdmi_buffer_read_id;
    }
    else
    {
        hdmiSourceAddr = temp_mva_w;
    }
    ...
    //DISP_InitVsyncTimer(true);

    HDMI_LOG("hdmi_resume done \n");
