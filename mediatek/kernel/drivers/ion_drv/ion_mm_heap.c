                                      ...
                                      void *unused)
{
...

    {
        ...
        seq_printf(s, "----------------------------------------------------\n");
        seq_printf(s, "%8.s %4.s %3.s %3.s %3.s %8.s %3.s %3.s %3.s %8.s " \
                        "%4.s %4.s %4.s %4.s %s\n", 
                    "size", "kmap", "ref","hdl","mod", "mva", "sec", "coh", "pid", "comm",
        ...
        for (n = rb_first(&dev->buffers); n; n = rb_next(n)) {
            ...
            seq_printf(s, "%8u %3d %3d %3d %3d %8x %3d %3d %3d %s " \
    ...
    case ION_MM_SET_DEBUG_INFO:
        {
            ...
            {
                buffer = ion_handle_buffer(Param.buf_debug_info_param.handle);
                if (buffer->heap == g_ion_heaps[ION_HEAP_IDX_MULTIMEDIA])
                {
                    ion_mm_buffer_info* pBufferInfo = buffer->priv_virt;
                    mutex_lock(&ion_mm_buffer_info_mutex);
