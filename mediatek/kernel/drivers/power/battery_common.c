...
unsigned int system_off_count = 0;	/* allan add on 20131120 */
...
{	
    ...
    if ((BMT_status.temperature < MIN_CHARGE_TEMPERATURE) || (BMT_status.temperature == ERR_CHARGE_TEMPERATURE))
    {
		...
		BMT_status.charger_protect_status = BATTERY_OVER_TEMP;	/* allan modify start on  20131224 */
        status = PMU_STATUS_FAIL;       
    }
    #endif                
...
static void mt_battery_thermal_check(void)
{
	if( (g_battery_thermal_throttling_flag==1) || (g_battery_thermal_throttling_flag==3) )
    {
        ...
        //ignore default rule
#else    
		if((BMT_status.temperature >= 60)||(BMT_status.temperature <= -20))	/* allan modify on 20131024 */
        {
            #if defined(CONFIG_POWER_EXT)
...
CHARGER_TYPE mt_charger_type_detection(void)
{
    ...
    if((BMT_status.charger_type == CHARGER_UNKNOWN) || (BMT_status.charger_type == NONSTANDARD_CHARGER))	/* allan modify on 20131224 */
    {
