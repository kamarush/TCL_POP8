...
int force_get_tbat(void)
{
    ...
    {   
		...
		//bat_temperature_val = bat_temperature_val + 1;	/* allan modify start on  20131224 */
    }
...
{
	//bm_print(BM_LOG_CRTI, "fgauge_get_saddles : allan add on !!!!!!!!!!!!!!!!\r\n");
		...
		if(battery_compatible == BAT_FEIMT_POP8)
	    	return sizeof(feimt_battery_profile_t2) / sizeof(BATTERY_PROFILE_STRUC);
		else if(battery_compatible == BAT_MBELL_POP8)
			return sizeof(mbell_battery_profile_t2) / sizeof(BATTERY_PROFILE_STRUC);
		else
			return sizeof(battery_profile_t2) / sizeof(BATTERY_PROFILE_STRUC);
	#endif
...
{
	//bm_print(BM_LOG_CRTI, "fgauge_get_saddles_r_table : allan add on !!!!!!!!!!!!!!!!\r\n");
#ifdef BATTERY_COMPATIBLE	/* allan add start on 20131122 */
	#ifdef POP7_BATTERY_COMPATIBLE	//pop7
		if(battery_compatible == BAT_FEIMT_POP7)
...
{
	//bm_print(BM_LOG_CRTI, "fgauge_get_profile : allan add on !!!!!!!!!!!!!!!!\r\n");
    switch (temperature)
    {
    #ifdef BATTERY_COMPATIBLE	/* allan add start on 20131122 */
		#ifdef POP7_BATTERY_COMPATIBLE	//pop7
...
{
	//bm_print(BM_LOG_CRTI, "fgauge_get_profile_r_table : allan add on !!!!!!!!!!!!!!!! battery_compatible = %d\r\n", battery_compatible);
    switch (temperature)
    {
    #ifdef BATTERY_COMPATIBLE	/* allan add start on 20131122 */
		#ifdef POP7_BATTERY_COMPATIBLE	//pop7
...
{
	//bm_print(BM_LOG_CRTI, "fg_qmax_update_for_aging : allan add on !!!!!!!!!!!!!!!!\r\n");
    ...
    if(hw_charging_done == KAL_TRUE) // charging full, g_HW_Charging_Done == 1
    {
        if(gFG_DOD0 > 85)
        {
            ...
            if(gFG_BATT_CAPACITY_aging == 0)
            {
					...
					if(battery_compatible == BAT_FEIMT_POP7)
						gFG_BATT_CAPACITY_aging = FEIMT_Q_MAX_POS_25;
					else if(battery_compatible == BAT_MBELL_POP7)
						gFG_BATT_CAPACITY_aging = MBELL_Q_MAX_POS_25;
					else	
...
void oam_init(void)
{
    ...
    ret = battery_meter_ctrl(BATTERY_METER_CMD_GET_HW_OCV, &gFG_voltage);
	//bm_print(BM_LOG_CRTI, "oam_init : allan add on !!!!!!!!!!!!!!!!\r\n");
...
void oam_run(void)
{
	...
	bm_print(BM_LOG_CRTI, "[oam_init_inf] %d, %d, %d, %d, %d, %d\n",
	 ...
	 bm_print(BM_LOG_CRTI, "[oam_run_inf] %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d\n", 
	  ...
	  //bm_print(BM_LOG_CRTI, "[bat_id_vol] %d\n", bat_id_vol); 
}
...
void battery_meter_check_bat_id(void)
{
	...
	else if ((val >= BAT_OTHER_POP8_VALUE)&&(val < BAT_MBELL_POP8_VALUE))
		battery_compatible = BAT_MBELL_POP8;
	else
...
static int battery_meter_resume(struct platform_device *dev)
{
    ...
    {    
		...
		{	
			if (( MIN_SUSPEND_CURRENT <= suspend_current) && (suspend_current < MAX_SUSPEND_CURRENT))
				suspend_current = suspend_current + SUSPEND_CURRENT_SETP;
		}
		else if ((g_sleep_fg_soc - g_sleep_fg_ui_soc) > SLEEP_AFTER_FG_DIFF)
		{
			if (( MIN_SUSPEND_CURRENT < suspend_current) && (suspend_current <= MAX_SUSPEND_CURRENT))
				suspend_current = suspend_current - SUSPEND_CURRENT_SETP;
		}
    	...
    	if(rtc_time_after_sleep - g_rtc_time_before_sleep > 3600)	//1hr
    	{
    		...
    		else
    		{
				 ...
				 oam_car_delta = suspend_current* (rtc_time_after_sleep - g_rtc_time_before_sleep + suspend_oam_time)/3600;
				 suspend_oam_time = suspend_current* (rtc_time_after_sleep - g_rtc_time_before_sleep + suspend_oam_time)%3600;
