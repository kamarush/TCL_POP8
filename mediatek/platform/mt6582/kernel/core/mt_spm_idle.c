...
static void PCM_Init(void)
{
    ...
    spm_write(SPM_CORE0_WFI_SEL, 0x0); // core_0 wfi_sel
    spm_write(SPM_CORE1_WFI_SEL, 0x0); // core_1 wfi_sel
    spm_write(SPM_CORE2_WFI_SEL, 0x0); // core_2 wfi_sel 
    spm_write(SPM_CORE3_WFI_SEL, 0x0); // core_3 wfi_sel 
...
{   
        ...
        if(core_id == 0)
        {
            ...
            spm_write(SPM_CORE0_WFI_SEL, SPM_MCDI_CORE_SEL);
        ...
        else // Core 1,2,3  Keep original IRQ
        {
            ...
            switch (core_id)
            {
                case 1 : spm_write(SPM_CORE1_WFI_SEL, SPM_MCDI_CORE_SEL); break;                     
                case 2 : spm_write(SPM_CORE2_WFI_SEL, SPM_MCDI_CORE_SEL); break;                     
                case 3 : spm_write(SPM_CORE3_WFI_SEL, SPM_MCDI_CORE_SEL); break;                     
                default : break;
            }
...
void spm_mcdi_LDVT_sodi(void)
{
	...
	if(res)
	{
			  printk("spm_mcdi_LDVT_sodi, get wd api error\n");
	}
	else
	{
...
MODULE_DESCRIPTION("MT6582 SPM-Idle Driver v0.1");
