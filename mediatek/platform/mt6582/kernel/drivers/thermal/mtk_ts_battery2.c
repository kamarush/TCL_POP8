...
static INT16 BattThermistorConverTemp(INT32 Res)
{
    ...
    if(g_Rbat_ntc_table == 1){//BAT_NTC_BL197
    ...
    else if(g_Rbat_ntc_table == 2){//BAT_NTC_TSM_1
		BATT_TEMPERATURE Batt_Temperature_Table[] = {
			{-20,70603},
			{-15,55183},
			{-10,43499},
    ...
    else if(g_Rbat_ntc_table == 3){//BAT_NTC_10_SEN_1
		BATT_TEMPERATURE Batt_Temperature_Table[] = {
			 {-20,74354},
			 {-15,57626},
			 {-10,45068},
	...
	else if(g_Rbat_ntc_table == 4){//BAT_NTC_10
		BATT_TEMPERATURE Batt_Temperature_Table[] = {
	        {-20,68237},
	        {-15,53650},
	        {-10,42506},
	...
	else if(g_Rbat_ntc_table == 5){//BAT_NTC_47
    BATT_TEMPERATURE Batt_Temperature_Table[] = {
	        {-20,483954},
	        {-15,360850},
	        {-10,271697},
...
static ssize_t mtktsbattery2_param_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
	...
	if (sscanf(desc, "%s %d %s %d %s %d %s %d",pull_R, &valR, pull_V, &valV, overcrilow, &over_cri_low,NTC_TABLE,&ntc_table ) == 8)
	{
		...
		if (err){
			printk("mtktsbattery2_register_thermal err\n");
            return -EINVAL;
		}
		return count;
	}
...
#ifdef INPUT_PARAM_FROM_USER
		entry = create_proc_entry("mtktsbattery2_param", S_IRUGO | S_IWUSR, mtktsbattery2_dir);
		if (entry)
		{
